...
const excluirLivro = async (req, res) => {
    ...
    try {
        ...
        const { rowCount: emprestimos } = await conexao.query('select * from emprestimos where livro_id = $1', [id]);
        
        
        if(emprestimos !== 0){
            return res.status(400).json(`Não foi possível excluir o livro ${livro.rows[0].nome} pois existem emprestimos registrados em seu id.`);
        }
