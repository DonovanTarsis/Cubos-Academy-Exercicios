...
const excluirAutor = async (req, res) => {
    ...
    try {
        ...
        const { rowCount: livros } = await conexao.query('select * from livros where autor_id = $1', [id]);
        
        
        if(livros !== 0){
            return res.status(400).json(`Não foi possível excluir o autor ${autor.rows[0].nome} pois existem livros cadastrados em seu nome.`);
        }
